...
def generate_transition_matrices(inp):
    ...
    for edge in edges:
        ...
        # e_weights_np = [str_2_phas_1(wei, ploidy) for wei in e_weights.keys()]
        # sorted_e_weights_np = []
        # for mtx in e_weights_np:
        #     sorted_matrix = mtx[np.argsort([''.join(map(str, row)) for row in mtx])]
        #     sorted_e_weights_np.append(sorted_matrix)
        # e_weights_str = [phas_2_str(pm) for pm in sorted_e_weights_np]
        new_e_weights = {}
        for key, val in e_weights.items():
            mtx = str_2_phas_1(key, ploidy)
            sorted_matrix = mtx[np.argsort([''.join(map(str, row)) for row in mtx])]
            new_e_weights[phas_2_str(sorted_matrix)] = val
        ...
        for i, ffstr in enumerate(source_phasings):
            for j, sfstr in enumerate(target_phasings):
                ...
                this_weight = np.sum([new_e_weights[pm] for pm in matched_phasings_str if pm in new_e_weights.keys()])
