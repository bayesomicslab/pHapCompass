...
def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="pHapCompass main entry point for short- and long-read haplotype assembly."
    )

    # Inputs
    parser.add_argument(
        "--bam-path", type=str, default=None,
        help="Path to BAM file. If provided with --vcf-path and no --fragment-file-path, extractHAIRS will be run."
    )
    parser.add_argument(
        "--frag-path", type=str, default=None,
        help="Path to fragment file (.frag). If provided, BAM is optional."
    )
    parser.add_argument(
        "--vcf-path", type=str, required=True,
        help="Path to VCF file (required: used for genotypes and/or ploidy inference)."
    )
    parser.add_argument(
        "--ploidy", type=int, default=None,
        help="Ploidy (e.g., 2, 3, 4). If not provided, inferred from VCF."
    )
    parser.add_argument(
        "--result-path", type=str, required=True,
        help="Output path (file path or prefix) for saving final haplotypes/results."
    )
    parser.add_argument(
        "--epsilon", type=float, default=0.00001,
        help="Sequencing error rate ε (used by both models). Default: 0.00001"
    )
    parser.add_argument(
        "--data-type", type=str, choices=["short", "long"], required=True,
        help="Type of data: 'short' for short reads, 'long' for long reads."
    )

    # Uncertainty (optional int, default 3 if flag used without value)
    parser.add_argument(
        "--uncertainty", nargs="?", const=3, type=int, default=None,
        help=("Enable uncertainty quantification. Optional value specifies the number of haplotype samples "
              "(default: 3 if flag is provided without a value).")
    )
    ...
    parser.add_argument("--verbose", action="store_true",
                        help="Enable verbose mode (equivalent to --log-level DEBUG).")
...
def assemble_main(argv=None) -> None:
...
def main(argv=None) -> None:
    """
    Keep the existing CLI intact, but allow:
      phapcompass simulation ...
      phapcompass eval ...
    """
    if argv is None:
        argv = sys.argv[1:]
    ...
    # Dispatch: simulation
    if len(argv) > 0 and argv[0] in {"simulation", "sim"}:
        from phapcompass.sim_cli import main as sim_main
        sim_main(argv[1:])
        return

    # Dispatch: evaluation
    if len(argv) > 0 and argv[0] in {"eval", "evaluate", "evaluation"}:
        from phapcompass.eval_cli import main as eval_main
        eval_main(argv[1:])
        return

    # Default: assembly (current behavior)
    assemble_main(argv)
